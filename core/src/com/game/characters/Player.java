...
public class Player extends Sprite {
	...
	private final float timeBetweenAtkSFX = 1.4f;

	private float atk = 0.9f;
	
	private final float timeBetweenStep = 1.5f;
	
	private float step = 1f;

	private Sound slashing, attacking, walking, getHit;

	// Character sprite sheet
	private TextureRegion[] walkUp, walkDown, walkLeft, walkRight, idleRight, idleLeft, idleUp, idleDown, attackUp1,
			attackUp2, attackUp3, attackDown1, attackDown2, attackDown3, attackLeft1, attackLeft2, attackLeft3,
			attackRight1, attackRight2, attackRight3;

	// Character animation
	private Animation<TextureRegion> walkUpAni, walkDownAni, walkLeftAni, walkRightAni, idleUpAni, idleDownAni,
			idleLeftAni, idleRightAni, attackUpAni1, attackUpAni2, attackUpAni3, attackDownAni1, attackDownAni2,
			attackDownAni3, attackLeftAni1, attackLeftAni2, attackLeftAni3, attackRightAni1, attackRightAni2,
			attackRightAni3;
	...
	// Character States
	public enum State {
		IDLE_UP, IDLE_RIGHT, IDLE_LEFT, IDLE_DOWN, WALK_UP, WALK_LEFT, WALK_RIGHT, WALK_DOWN, WALK_LEFT_DOWN,
		WALK_RIGHT_DOWN, WALK_LEFT_UP, WALK_RIGHT_UP, ATTACK_UP, ATTACK_DOWN, ATTACK_LEFT, ATTACK_RIGHT
	}
	...
	public Player(float hp, float speed, float damage, Texture walkUpTexture, Texture walkDownTexture,
			Texture walkLeftTexture, Texture walkRightTexture, Texture attackUpTexture, Texture attackDownTexture,
			Texture attackRightTexture, Texture attackLeftTexture) {
		this.hp = hp;
		this.speed = speed;
		this.damage = damage;

		slashing = Gdx.audio.newSound(Gdx.files.internal("SFX/27_sword_miss_2.wav"));

		attacking = Gdx.audio.newSound(Gdx.files.internal("SFX/attack3.wav"));
		
		walking = Gdx.audio.newSound(Gdx.files.internal("SFX/Grass 1.wav"));
		
		getHit = Gdx.audio.newSound(Gdx.files.internal("SFX/damaged2.wav"));
	...
	public void attackUp2() {
		...
		if (atk > timeBetweenAtkSFX) {
			atk = 0f;
			slashing.play(0.2f);
			attacking.play(0.2f);
		} else {
			atk += Gdx.graphics.getDeltaTime();
		}
	}
	...
	public void getHit(float damage) {
		...
		if (currentState == State.IDLE_UP || currentState == State.ATTACK_UP || currentState == State.WALK_UP) {
			setPosition(getX(), getY() - 32);
		} else if (currentState == State.IDLE_LEFT || currentState == State.ATTACK_LEFT || currentState == State.WALK_LEFT
				|| currentState == State.WALK_LEFT_UP || currentState == State.WALK_LEFT_UP) {
			setPosition(getX() + 32, getY());
		} else if (currentState == State.IDLE_RIGHT || currentState == State.ATTACK_RIGHT || currentState == State.WALK_RIGHT
				|| currentState == State.WALK_RIGHT_UP || currentState == State.WALK_RIGHT_UP) {
			setPosition(getX() - 32, getY());
		} else if (currentState == State.IDLE_DOWN || currentState == State.ATTACK_DOWN || currentState == State.WALK_DOWN) {
			setPosition(getX(), getY() + 32);
		}
	}
